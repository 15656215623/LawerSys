...
function load_data() {
	...
	//显示头像
	//根据 identity来决定显示的个人中心
	var identity = localStorage.getItem("identity");
	if(identity=="user"){
		$("#mycenter").attr("href","usercenter.jsp"); 
	}else if(identity=="admin"){
		$("#mycenter").attr("href","Admin.jsp"); 
	}else if(identity=="layer"){
		$("#mycenter").attr("href","layer.jsp"); 
	}else{
		alert("登录之后可以享受更多的权益"); 
	}
	//根据 identity来决定显示的个人中心
		...
		success : function(ret) {
			...
			if(ret=="fail"){
				...
				//关闭模态框
				 ...
				 //把用户和律师分开，用一个字段
				 localStorage.setItem("identity","layer");	
				 //把logo拿出来
				 localStorage.setItem("userlogo",ret);
				 //页面开始跳转，跳转到后台
